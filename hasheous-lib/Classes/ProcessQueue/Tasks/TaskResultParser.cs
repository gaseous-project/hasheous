...
namespace Classes.ProcessQueue
{
    ...
    public class TaskResultParser : IQueueTask
    {
        ...
        public async Task<object?> ExecuteAsync()
        {
            ...
            {
                Console.WriteLine($"Error in TaskResultParser: {ex.Message}");
            }
            ...
            try
            {
                ...
                foreach (var client in clients)
                {
                    if (client.IsStale)
                    {
                        ...
                        {
                            Console.WriteLine($"Error unregistering stale client ID {client.Id}: {ex.Message}");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine($"Error pruning stale clients: {ex.Message}");
            }
            ...
            try
            {
                ...
                string resetSql = "SELECT * FROM Task_Queue WHERE (status = @inProgressStatus AND completion_time < @inProgressTime) OR (status = @assignedStatus AND completion_time < @assignedTime);";
                ...
                foreach (DataRow row in staleTasks.Rows)
                {
                    ...
                    if (staleTask != null)
                    {
                        ...
                        {
                            Console.WriteLine($"Error resetting stale task ID {staleTask.Id}: {ex.Message}");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine($"Error resetting stale tasks: {ex.Message}");
            }
