...
- Task worker polling depends on the `Task_Queue` status/client/start-time index and the `UPDATE ... WHERE id IN (SELECT ... FOR UPDATE SKIP LOCKED)` + equality `SELECT` pattern; changing those fields without keeping the same transaction shape can trigger command timeouts under concurrent polling.
