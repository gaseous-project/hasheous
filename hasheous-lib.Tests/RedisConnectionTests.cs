...
public class RedisConnectionTests
{
    ...
    [Fact]
    public void SmallComplexValueUsesFramedPlainJson()
    {
        var value = new CacheValue { Name = "small", Count = 42 };

        byte[] payload = SerializeComplexCacheValue(value);
        CacheValue? roundTripped = DeserializeComplexCacheValue<CacheValue>(payload);
    ...
    [Fact]
    public void LargeCompressibleComplexValueUsesFramedBrotli()
    {
    ...
    public void UnframedOrUnknownPayloadIsRejected(byte[] payload)
    {
        TargetInvocationException wrapped = Assert.Throws<TargetInvocationException>(
            () => DeserializeComplexCacheValue<CacheValue>(payload));
        Assert.IsType<InvalidDataException>(wrapped.InnerException);
    }
    ...
    private static T? DeserializeComplexCacheValue<T>(byte[] payload)
    {
        return (T?)GetRedisMethod("DeserializeComplexCacheValue").MakeGenericMethod(typeof(T)).Invoke(null, new object[] { (ReadOnlyMemory<byte>)payload });
    }
